...
ACP8may <- "C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACPWeeklyEnroll.xlsx"
...
acp_zip <- read_excel("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Enrollments-by-Zip-as-of-April-1-2023.xlsx", sheet = 1, col_names = TRUE) %>% clean_names() %>% rename("enrollments" = "subscribers_in_zip") %>% select(zipcode, enrollments) %>% 
...
claims2023 <- read_excel("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-Zip-January-March-2023.xlsx") 

claims22_23 <- bind_rows(claims2022,claims2023)

acp_zip_spent <- claims22_23 %>% clean_names() %>% select(zipcode, total_support) %>%
  mutate(zipcode = str_pad(zipcode, width = 5, side = c("left"), pad = 0)) %>%
  group_by(zipcode) %>% 
  summarize(total_support = sum(total_support))

#write this to the dashboard folder each time to use for other claims-related elements
write.csv(acp_zip_spent,"C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_claims22_23.csv")
...
ACPstates <-read_excel(path = ACP8may, sheet = 4, col_names = TRUE) %>%
  select(State,HH_8may23) %>% 
...
#-------------------------------------------------------
#NATIONAL DATA
acp_national <-read_excel(path = ACP8may, sheet = 2, col_names = TRUE) %>% clean_names() %>%
...
future <- data.frame(monthBYyear = c(26:238))
...
HHincome_state.3 <- HHincome_state.2 %>% left_join(y = totalHH_state, by = "geoid") %>% #join with total # households
  ...
  HHEoE_perc = (HH_8may23/hh_qualify)*100, ###NEED TO EDIT THIS ROW ANY TIME STATE DATA IS UPDATED###
  HH_eligiblePUMS = (hh_eligible/total_hh)*100,
  HHEoE_PUMSperc = (HH_8may23/hh_eligible)*100,
  adj_factor = HHEoE_perc/HHEoE_PUMSperc) %>% 
  rename("totalHH"="total_hh", "HHqualify"="hh_qualify") %>%
  select(-name, -state) %>%
  select(geoid, State, totalHH, HHqualify, HHqualify_perc, HH_8may23, HHEoE_perc,hh_eligible, HH_eligiblePUMS, HHEoE_PUMSperc, adj_factor) %>% 
...
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_ZipStateCD_8may2023", data = ACP_ZipStateCD_geo)


# #write to git folder
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_DASHBOARD_ZipStateCD_8may2023.shp", data = ACP_ZipStateCD_geo, overwrite = TRUE)
...
```{r aggregate zipcode data to congressional district, echo = FALSE}
...
#import the list of zcta's for AK and HI
ak.cd <- congressional_districts(cb = FALSE, state = 02) #load AK zcta's
...
ACP_zip2cd <- HHincome_cd.3  %>%
  ...
  mutate(state_code = substr(geoid, 1, 2)) %>% left_join(y = fips_states, by = "state_code")
  

#summarize zip data within districts
ACP_cd <- ACP_zip2cd %>% mutate(cd_enrolled = res_ratio*enrollments, cd_support = res_ratio*total_support) %>%
...
ACP_cd_info <- ACP_cd %>% mutate(district = substr(geoid, 3,4)) %>% left_join(y = legislators_188_simple, by = c("state", "district")) #add this dataset to ACP_DASHBOARD_V2 (ACP_cd_infoSP gets used for map)

#congressional district spatial
cd <- congressional_districts(cb = FALSE, year = 2020) 

ACP_cd_infoSP<- cd %>% left_join(y = ACP_cd_info, by = c("GEOID" = "geoid"))

#write to arcgis to join with cd spatial & write as shp to data folder
#arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_CD8may23", data = ACP_cd_infoSP)

# #write to git folder
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_CD8may23.shp", data = ACP_cd_infoSP, overwrite = TRUE)
...
acp_ToPlot1 <- data.frame(acp_predict1$fit) %>% mutate(monthBYyear = 26:238) %>% rename(enrolled = fit)
...
arc.write("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_ZipData_8may.shp", data = acp.zip.sp, overwrite = TRUE) #may need to edit in future to ACP_ZipStateCD_geo
...
HHincome_state.3b <- HHincome_state.3 %>% mutate(estimated_spent = HH_8may23*30*1.025) %>% 
   rename("perc_eligible" = "HH_eligiblePUMS", "perc_EoE" = "HHEoE_PUMSperc") %>%
  select(-HHqualify, -HHqualify_perc, -HHEoE_perc) 

wb <- loadWorkbook("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_8may23.xlsx")
...
saveWorkbook(wb,"C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_8may23.xlsx",overwrite = TRUE)
