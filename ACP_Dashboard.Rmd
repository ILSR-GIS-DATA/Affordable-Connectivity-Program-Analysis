...
ACP12sep <- "G:/My Drive/ACPdashboard/ACPWeeklyEnroll.xlsx"
...
ACPstates <-read_excel(path = ACP12sep, sheet = 4, col_names = TRUE) %>%
  select(State,HH_sep12)%>% 
...
#NATIONAL DATA
acp_national <-read_excel(path = ACP12sep, sheet = 2, col_names = TRUE) %>% clean_names() %>%
...
arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_ZipBenton13sep2022", data = ACPzipBenton, overwrite = TRUE)
...
HHincome_state.3 <- HHincome_state.2 %>% left_join(y = totalHH_state, by = "geoid") %>%
  ...
  mutate(HHqualify_perc = (hh_qualify/total_hh)*100, 
  HHEoE_perc = (HH_sep12/hh_qualify)*100) %>% ###NEED TO EDIT THIS ROW ANY TIME STATE DATA IS UPDATED###
  rename("totalHH"="total_hh", "HHqualify"="hh_qualify") %>%
  select(-geoid, -name, -state) %>%
  select(State, totalHH, HHqualify, HHqualify_perc, HH_sep12, HHEoE_perc)%>% 
