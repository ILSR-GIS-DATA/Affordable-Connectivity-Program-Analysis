...
ACP29may <- "C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACPWeeklyEnroll.xlsx"
...
claims2023 <- read_excel("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-Zip-January-April-2023.xlsx") 
...
acp_zip_spent <- claims22_23 %>% clean_names() %>% select(zipcode,device_support, total_support) %>%
  ...
  group_by(zipcode) %>% 
  summarize(total_support = sum(total_support), device_support = sum(na.omit(device_support)))
...
ACPstates <-read_excel(path = ACP29may, sheet = 4, col_names = TRUE) %>%
  select(State,HH_29may23) %>% 
...
#-------------------------------------------------------
#NATIONAL DATA
acp_national <-read_excel(path = ACP29may, sheet = 2, col_names = TRUE) %>% clean_names() %>%
...
future <- data.frame(monthBYyear = c(27:238))
...
HHincome_state.3 <- HHincome_state.2 %>% left_join(y = totalHH_state, by = "geoid") %>% #join with total # households
  ...
  HHEoE_perc = (HH_29may23/hh_qualify)*100, ###NEED TO EDIT THIS ROW ANY TIME STATE DATA IS UPDATED###
  HH_eligiblePUMS = (hh_eligible/total_hh)*100,
  HHEoE_PUMSperc = (HH_29may23/hh_eligible)*100,
  adj_factor = HHEoE_perc/HHEoE_PUMSperc) %>% 
  rename("totalHH"="total_hh", "HHqualify"="hh_qualify") %>%
  select(-name, -state) %>%
  select(geoid, State, totalHH, HHqualify, HHqualify_perc, HH_29may23, HHEoE_perc,hh_eligible, HH_eligiblePUMS, HHEoE_PUMSperc, adj_factor) %>% 
...
# #write to git folder
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_DASHBOARD_Benton_29may2023.shp", data = ACP_Benton_sp, overwrite = TRUE)
...
cd_data <- read_excel("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Funding-Summary-by-Geography_16May23.xlsx", sheet = "CD_MAR") %>% clean_names()
...
ACP_116to118cd <- cd116TO118  %>% 
 left_join(y = HHincome_cd.3, by = c("cd116" = "geoid")) %>%  #join conversion file with eligibility data derived from ACS (116th)
  ...
  rename("state_code" = "state","state" = "stab", "district" = "cd118")
  
ACP_cd <- ACP_116to118cd %>% full_join(y = cd_data_all, by = c("district", "state_code", "state")) %>% #join with enrollment/claims data
...
ACP_cd_info <- legislators_188_simple %>% left_join(y = ACP_cd, by = c("state", "district" = "cd")) #add this dataset to ACP_DASHBOARD_V2 (ACP_cd_infoSP gets used for map)



#write to arcgis to join with cd spatial & write as shp to data folder (don't use tigris, unless it changes to 118)
#arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_CD29may23", data = ACP_cd_info, overwrite = TRUE)

# #write to git folder
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_CD29may23.shp", data = ACP_cd_info, overwrite = TRUE)
...
acp_ToPlot1 <- data.frame(acp_predict1$fit) %>% mutate(monthBYyear = 27:238) %>% rename(enrolled = fit)
...
enroll75b <- enroll75 %>% bind_rows(enroll75[rep(nrow(enroll75), 36),] %>%
              ...
              mutate(month.spent = enrolled*30, cum.spent = cumsum(month.spent),
                     perc.enrolled = (enrolled/US_eligibleHH)*100 ,perc.used = (cum.spent /ebb.acp)*100,
                     month = seq(as.Date("2022-01-01"), by = "month",length.out = 265)) %>%
  select(month, enrolled, lwr, upr,perc.enrolled,month.spent,cum.spent, perc.used) %>%
...
arc.write("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_ZipData_29may.shp", data = acp.zip.sp, overwrite = TRUE) #may need to edit in future to ACP_ZipStateCD_geo
...
HHincome_state.3b <- HHincome_state.3 %>% mutate(estimated_spent = HH_29may23*30*1.025) %>% 
   rename("perc_eligible" = "HH_eligiblePUMS", "perc_EoE" = "HHEoE_PUMSperc") %>%
  select(-HHqualify, -HHqualify_perc, -HHEoE_perc) 

wb <- loadWorkbook("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_29may23.xlsx")
...
saveWorkbook(wb,"C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_29may23.xlsx",overwrite = TRUE)
