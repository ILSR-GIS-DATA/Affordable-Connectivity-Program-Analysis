...
ACP_ClaimsData <- read_excel("G:/My Drive/ACPdashboard/ACP-Claims-by-Zip-January-September-2022.xlsx", 
...
claims <- ACP_ClaimsData %>% select("data_month","Zip Code","total_claimed_subscribers") %>%
  rename("month" = "data_month", "zipcode" = "Zip Code", "claimed_subsc" = "total_claimed_subscribers")
...
ACP_HHData <- read_excel("G:/My Drive/ACPdashboard/ACP-Households-by-Zip-January-September-2022.xlsx", 
...
ACP14nov <- "G:/My Drive/ACPdashboard/ACPWeeklyEnroll.xlsx"
...
acp_zcta <- read_excel("C:/Users/chris/Dropbox (ILSR)/Christine Parker/RawData/ACP/ACP-Enrollments-by-Zip-Code-as-of-October-1-2022.xlsx", sheet = 1, col_names = TRUE) %>% clean_names() %>% rename("enrollments" = "subscribers_in_zip") %>% select(zipcode, enrollments)
#make sure all zipcodes have leading zeros & convert to character
...
ACPstates <-read_excel(path = ACP14nov, sheet = 4, col_names = TRUE) %>%
  select(State,HH_nov14) %>% 
...
#-------------------------------------------------------
#NATIONAL DATA
acp_national <-read_excel(path = ACP14nov, sheet = 2, col_names = TRUE) %>% clean_names() %>%
...
future <- data.frame(monthBYyear = c(20:240))
...
arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_ZipBenton16Nov2022", data = ACPzipBenton, overwrite = TRUE)
...
HHincome_state.3 <- HHincome_state.2 %>% left_join(y = totalHH_state, by = "geoid") %>%
  ...
  mutate(HHqualify_perc = (hh_qualify/total_hh)*100, 
  HHEoE_perc = (HH_nov14/hh_qualify)*100) %>% ###NEED TO EDIT THIS ROW ANY TIME STATE DATA IS UPDATED###
  rename("totalHH"="total_hh", "HHqualify"="hh_qualify") %>%
  select(-name, -state) %>%
  select(geoid, State, totalHH, HHqualify, HHqualify_perc, HH_nov14, HHEoE_perc) %>% 
...
acp_ToPlot1 <- data.frame(acp_predict1$fit) %>% mutate(month = 12:232) %>% rename(enrolled = fit)
...
enroll45 <- ACP.only %>%  bind_rows(add.in45) %>% arrange(month) %>% as.data.frame()
enroll45b <- enroll45 %>% bind_rows(enroll45[rep(nrow(enroll45), 36),] %>%
              ...
              mutate(lwr = NA, upr = NA,
                     ...
                     month = seq(as.Date("2022-01-01"), by = "month",length.out = 48)) %>%
  select(month, enrolled, lwr, upr,perc.enrolled,month.spent,cum.spent, perc.used) %>%
  mutate(month.spent25 = month.spent * 1.025,cum.spent25 = cumsum(month.spent25), perc.used25 = (cum.spent25 /ebb.acp)*100, model = "45 Percent Enrollment (14.7 Million Households)")
...
enroll55 <- acp_ToPlot1 %>% subset(enrolled < US_eligibleHH55) %>% bind_rows(ACP.only) %>% arrange(month) 
...
enroll60 <- acp_ToPlot1 %>% subset(enrolled < US_eligibleHH60) %>% bind_rows(ACP.only) %>% arrange(month) 
...
xlsx::write.xlsx("G:/My Drive/ACPdashboard/ACP_DASHBOARD.xlsx", sheetName = "ModelData", x = modeldata, append = TRUE)
