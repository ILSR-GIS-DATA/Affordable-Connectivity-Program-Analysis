...
```{r Load & Edit Data, echo=FALSE, cache=TRUE, cache.lazy=FALSE, results='hide'}

#load ACP subcriber & claim data (zip code) - these data are updated infrequently
ACP_ClaimsData <- read_excel("G:/My Drive/ACPdashboard/ACP_ClaimsData_Jan-Aug-2022.xlsx", 
    sheet = "RawData", col_types = c("date", 
...
claims <- ACP_ClaimsData %>% select("Data Month","Zip Code","Total Claimed Subscribers", "Total Subscribers") %>%
  rename("month" = "Data Month", "zipcode" = "Zip Code", "claimed_subsc" = "Total Claimed Subscribers", "all_subsc" = "Total Subscribers")
...
claims_zcta <- claims %>% group_by(zipcode) %>% subset(month == max(month)) %>%#create dataset of claimed subscribers per zipcode for most recent month
  rename("claimed" = "claimed_subsc") %>% ungroup() %>% select(zipcode, claimed)
claims_US <- claims %>% group_by(zipcode) %>% subset(month == max(month)) %>% select(claimed_subsc,all_subsc) %>% ungroup() %>%
  summarize(total_claimed = sum(na.omit(claimed_subsc)), total_enrolled = sum(all_subsc))

#-------------------------------------------------------
#load & edit county claim data for state map
ACP_county_claims <- read_excel("G:/My Drive/ACPdashboard/ACP-Households-and-Claims-by-County-January-August-2022.xlsx", 
...
county_claims <- ACP_county_claims %>% select("Data Month","State FIPS","State Name","County Name", "Total Subscribers", "Total Claimed Subscribers") %>%
  rename("month" = "Data Month", "state_fips" = "State FIPS", "state_name" = "State Name","county" = "County Name","claimed" = "Total Claimed Subscribers", "subscribed" = "Total Subscribers") %>%
  mutate(state_name = str_to_sentence(state_name))

claims_state <- county_claims %>% group_by(county, state_fips) %>% subset(month == max(month)) %>%#create dataset of claimed subscribers per county/state for most recent month
...
ACP17oct <- "G:/My Drive/ACPdashboard/ACPWeeklyEnroll.xlsx"
...
ACPstates <-read_excel(path = ACP17oct, sheet = 4, col_names = TRUE) %>%
  select(State,HH_oct17) %>% 
...
#-------------------------------------------------------
#NATIONAL DATA
acp_national <-read_excel(path = ACP17oct, sheet = 2, col_names = TRUE) %>% clean_names() %>%
...
acp.claims <- claims_zcta %>% select(zipcode, claimed) %>% 
  ...
  rename("totalHH" = "total_hh") %>% 
  mutate(HHEoE_perc = ifelse(HHCoE_perc >= 100, 101,
...
acp.claimed_Benton2 <- acp.claims %>% left_join(y = bentonzips, by = "zipcode")
...
#write to git folder
arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_DASHBOARD_ZipData.shp", data = ACPzipBenton_claimed, overwrite = TRUE)
...
HHincome_state.3 <- HHincome_state.2 %>% left_join(y = totalHH_state, by = "geoid") %>%
  ...
  mutate(HHqualify_perc = (hh_qualify/total_hh)*100, 
  HHEoE_perc = (HH_oct17/hh_qualify)*100) %>% ###NEED TO EDIT THIS ROW ANY TIME STATE DATA IS UPDATED###
  rename("totalHH"="total_hh", "HHqualify"="hh_qualify") %>%
  select(-name, -state) %>%
  select(geoid, State, totalHH, HHqualify, HHqualify_perc, HH_oct17, HHEoE_perc) %>% 
...
acp_ToPlot1 <- data.frame(acp_predict1$fit) %>% mutate(month = 10:232) %>% rename(enrolled = fit)
