...
ACP24apr <- "C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACPWeeklyEnroll.xlsx"
...
acp_zcta <- read_excel("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Enrollments-by-Zip-Code-as-of-March-1-2023.xlsx", sheet = 1, col_names = TRUE) %>% clean_names() %>% rename("enrollments" = "subscribers_in_zip") %>% select(zipcode, enrollments)

#load total spent by zipcode (to join with congressional data)
acp_zcta_spent <- read_excel("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-Zip-January2022-Feb2023.xlsx", 
...
ACPstates <-read_excel(path = ACP24apr, sheet = 4, col_names = TRUE) %>%
  select(State,HH_24apr23) %>% 
...
#-------------------------------------------------------
#NATIONAL DATA
acp_national <-read_excel(path = ACP24apr, sheet = 2, col_names = TRUE) %>% clean_names() %>%
...
future <- data.frame(monthBYyear = c(25:238))
...
HHincome_state.3 <- HHincome_state.2 %>% left_join(y = totalHH_state, by = "geoid") %>% #join with total # households
  ...
  HHEoE_perc = (HH_24apr23/hh_qualify)*100, ###NEED TO EDIT THIS ROW ANY TIME STATE DATA IS UPDATED###
  HH_eligiblePUMS = (hh_eligible/total_hh)*100,
  HHEoE_PUMSperc = (HH_24apr23/hh_eligible)*100,
  adj_factor = HHEoE_perc/HHEoE_PUMSperc) %>% 
  rename("totalHH"="total_hh", "HHqualify"="hh_qualify") %>%
  select(-name, -state) %>%
  select(geoid, State, totalHH, HHqualify, HHqualify_perc, HH_24apr23, HHEoE_perc,hh_eligible, HH_eligiblePUMS, HHEoE_PUMSperc, adj_factor) %>% 
...
ACP_ZipStateCD_geo <- usZIP %>% left_join(y = ACP_ZipStateCD, by = c("GEOID20" = "zipcode"))



#arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_ZipStateCD24apr23", data = ACP_ZipStateCD)

#st_write("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_ZipStateCD24apr23.shp", obj = ACP_ZipStateCD_geo)
...
ACP_congdist_geo <- cd %>% left_join(y = ACP_congdist, by = c("GEOID" = "geoid_cd"))
...
#arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_CD24apr23b", data = ACP_congdist) 
#arc.write("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_CD_24apr23.shp", data = ACP_congdist_geo)
...
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_ZipStateCD_24apr2023", data = ACP_ZipStateCD_geo)
# 
# 
# 
# #write to git folder
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_DASHBOARD_ZipStateCD_24apr2023.shp", data = ACP_ZipStateCD_geo, overwrite = TRUE)
...
acp_ToPlot1 <- data.frame(acp_predict1$fit) %>% mutate(monthBYyear = 25:238) %>% rename(enrolled = fit)
...
enroll40b <- enroll40 %>% bind_rows(enroll40[rep(nrow(enroll40), 36),] %>%
              ...
              mutate(month.spent = enrolled*30, 
                     ...
                     perc.enrolled = (enrolled/US_eligibleHH)*100, 
                     perc.used = (cum.spent /ebb.acp)*100,
                     month = seq(as.Date("2022-01-01"), by = "month",length.out = 55)) %>%
  select(month, enrolled, lwr, upr,perc.enrolled,month.spent,cum.spent, perc.used) %>%
  mutate(month.spent25 = month.spent * 1.025,cum.spent25 = cumsum(month.spent25), perc.used25 = (cum.spent25 /ebb.acp)*100, model = "40 Percent Enrollment (20.8 Million Households)")
enroll40b$month <- format(enroll40b$month, format = "%b-%y")
...
enroll45b <- enroll45 %>% bind_rows(enroll45[rep(nrow(enroll45), 36),] %>%
              ...
              mutate(month.spent = enrolled*30, cum.spent = cumsum(month.spent),
                     perc.enrolled = (enrolled/US_eligibleHH)*100 ,perc.used = (cum.spent /ebb.acp)*100,
                     month = seq(as.Date("2022-01-01"), by = "month",length.out = 59)) %>%
  select(month, enrolled, lwr, upr,perc.enrolled,month.spent,cum.spent, perc.used) %>%
  mutate(month.spent25 = month.spent * 1.025,cum.spent25 = cumsum(month.spent25), perc.used25 = (cum.spent25 /ebb.acp)*100, model = "45 Percent Enrollment (23.5 Million Households)") 
enroll45b$month <- format(enroll45b$month, format = "%b-%y")
...
enroll50b <- enroll50 %>% bind_rows(enroll50[rep(nrow(enroll50), 36),] %>%
              ...
              mutate(month.spent = enrolled*30, cum.spent = cumsum(month.spent),
                     perc.enrolled = (enrolled/US_eligibleHH)*100 ,perc.used = (cum.spent /ebb.acp)*100,
                     month = seq(as.Date("2022-01-01"), by = "month",length.out = 63)) %>%
  select(month, enrolled, lwr, upr,perc.enrolled,month.spent,cum.spent, perc.used) %>%
  mutate(month.spent25 = month.spent * 1.025,cum.spent25 = cumsum(month.spent25), perc.used25 = (cum.spent25 /ebb.acp)*100, model = "50 Percent Enrollment (26.1 Million Households)") 
enroll50b$month <- format(enroll50b$month, format = "%b-%y")
...
arc.write("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_ZipData_24apr.shp", data = ACP_ZipStateCD_geo, overwrite = TRUE) #may need to edit in future to ACP_ZipStateCD_geo
...
HHincome_state.3b <- HHincome_state.3 %>% mutate(estimated_spent = HH_24apr23*30*1.025) %>% 
   rename("perc_eligible" = "HH_eligiblePUMS", "perc_EoE" = "HHEoE_PUMSperc") %>%
  select(-HHqualify, -HHqualify_perc, -HHEoE_perc) 

wb <- loadWorkbook("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_24apr23.xlsx")
...
saveWorkbook(wb,"C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_24apr23.xlsx",overwrite = TRUE)
