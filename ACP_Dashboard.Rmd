...
###DO NOT NEED TO RE-RUN UNTIL 2022 DATA ARE AVAILABLE- READS FROM CSV IN NEXT CHUNK###
#load pums data that was downloaded from FTP site
ha <- fread("C:/Users/chris/ILSR Dropbox/Christine Parker/Christine Parker/RawData/CensusDataQueries/New folder/psam_husa.csv")
#h_vars <- as.data.frame(names(ha))
hb <- fread("C:/Users/chris/ILSR Dropbox/Christine Parker/Christine Parker/RawData/CensusDataQueries/New folder/psam_husb.csv")
hc <- fread("C:/Users/chris/ILSR Dropbox/Christine Parker/Christine Parker/RawData/CensusDataQueries/New folder/psam_husc.csv")
hd <- fread("C:/Users/chris/ILSR Dropbox/Christine Parker/Christine Parker/RawData/CensusDataQueries/New folder/psam_husd.csv")
...
rm(h.data)
pa <- fread("C:/Users/chris/ILSR Dropbox/Christine Parker/Christine Parker/RawData/CensusDataQueries/New folder/psam_pusa.csv")
pb <- fread("C:/Users/chris/ILSR Dropbox/Christine Parker/Christine Parker/RawData/CensusDataQueries/New folder/psam_pusb.csv")
pc <- fread("C:/Users/chris/ILSR Dropbox/Christine Parker/Christine Parker/RawData/CensusDataQueries/New folder/psam_pusc.csv")
pd <- fread("C:/Users/chris/ILSR Dropbox/Christine Parker/Christine Parker/RawData/CensusDataQueries/New folder/psam_pusd.csv")
...
rm(eligible.all, h.data2, p.data2, p.filt, p.labeled)

write.csv(st_eligible, "C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/state_eligible.csv")


```

```{r Load & Edit Enrollment Data, echo=FALSE, cache=TRUE, cache.lazy=FALSE, results='hide'}
#load state eligible dataset created from chunk 2
st_eligible <- read.csv("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/state_eligible.csv", header = TRUE, sep = ,)
#make sure all state codes have leading zero
st_eligible$ST <-  str_pad(st_eligible$ST, width = 2, side = c("left"), pad = 0)
...
ACP8feb <- "C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACPWeeklyEnroll.xlsx"
...
acp_zip <- read_excel("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Enrollments-by-Zip-as-of-February-8-2024.xlsx", sheet = 1, col_names = TRUE) %>% clean_names() %>% rename("enrollments" = "total_subscribers") %>% select(zipcode, enrollments) %>% mutate(zipcode = str_pad(zipcode, width = 5, side = c("left"), pad = 0)) %>%
  right_join(y = zip2zcta, by = "zipcode")

##loads enrollment by county data (this dataset does not include subscription vs. device & claims details)
acp_county <- read_excel("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Enrollments-by-County-as-of-February-8-2024.xlsx", sheet = 1, col_names = TRUE) %>% clean_names() %>% rename("enrollments" = "total_subscribers") %>% select(county_name:enrollments) %>% mutate(geoid = paste0(state_fips,"",county_fips)) 

#load total spent by zipcode (to join with congressional data)
claims2022 <- read_excel("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-Zip-January-December-2022.xlsx")
claims2023 <-  read_excel("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-Zip-January-December-2023.xlsx", 
    col_types = c("date", "text", "numeric", 
        "numeric", "numeric", "numeric", 
        "numeric"),
    col_names = c("data_month", "zipcode", "total_claimed_subscribers", "total_claimed_devices", "service_support",
    "device_support", "total_support"))
claims2024 <-  read_excel("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-Zip-January-March-2024.xlsx", 
    col_types = c("date", "text", "numeric", 
        "numeric", "numeric", "numeric", 
        "numeric"),
    col_names = c("data_month", "zipcode", "total_claimed_subscribers", "total_claimed_devices", "service_support",
...
claims22_24 <- bind_rows(claims2022,claims2023, claims2024)

acp_zip_spent <- claims22_24 %>% clean_names() %>% select(zipcode,device_support, total_support) %>%
...
write.csv(claims22_24,"C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_claims22_24.csv")

# #load total spent by county 
# cnty_claims2022 <- read_excel("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-County-January-December-2022.xlsx")
# cnty_claims2023 <-  read_excel("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-County-January-December-2023.xlsx")
# cnty_claims2024 <-  read_excel("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-County-January-February-2024.xlsx")
#    
# 
# cnty_claims22_24 <- bind_rows(cnty_claims2022,cnty_claims2023, cnty_claims2024) %>% mutate(geoid = paste0(state_fips,"",county_fips))
# 
# acp_county_spent <- cnty_claims22_24 %>% clean_names() %>% select(state, county_name, geoid, claimed_device_support,claimed_service_support, total_claimed_support) %>%
#   group_by(state, county_name, geoid) %>% 
#   summarize(total_support = sum(na.omit(total_claimed_support)), device_support = sum(na.omit(claimed_device_support)), service_support = sum(na.omit(claimed_service_support)))
# 
# #write this to the dashboard folder each time to use for other claims-related elements
# write.csv(cnty_claims22_24,"C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_cnty_claims22_24.csv")
...
ACPstates <-read_excel(path = ACP8feb, sheet = 4, col_names = TRUE) %>%
  select(State,HH_8feb24) %>% 
...
#arcgisbinding::arc.write("C:/Users/chris/ILSR Dropbox/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/usZIP", data = usZIP, overwrite=TRUE)

#-------------------------------------------------------
#NATIONAL DATA
acp_national <-read_excel(path = ACP8feb, sheet = 2, col_names = TRUE) %>% clean_names() %>%
...
future <- data.frame(monthBYyear = c(35:238))
...
HHincome_state.1 <- get_acs(survey = "acs5", geography = "state", table = "B19001", year = 2022, geometry = FALSE, moe_level = 95, state = NULL,cache_table = TRUE) %>% clean_names()  #download version without geography 

#download household income data from acs
totalHH_state <- get_acs(survey = "acs5", geography = "state", variables = "B19001_001", year = 2022, geometry = FALSE, moe_level = 95, state = NULL,cache_table = TRUE) %>% clean_names() %>% select(geoid,estimate,moe) %>% rename("total_hh" = "estimate") %>%  #download version without geography 
mutate(total_hh = ifelse(total_hh == 0, moe, total_hh)) %>% select(-moe)
  
#download average household size from acs
HH.size_state <- get_acs(survey = "acs5", geography = "state", variables = "B25010_001", year = 2022, geometry = FALSE, moe_level = 95, state = NULL,cache_table = TRUE) %>% clean_names() %>% select(geoid,estimate) %>% rename("hh.size" = "estimate")
...
HHincome_state.2 <- HHincome_state.1  %>% select(-moe) %>% left_join(y = inc.max.vals, by = c("variable" = "var"))  %>%
 ...
 mutate(hh.size = if_else(is.na(hh.size) == TRUE, 2.57, hh.size), #for any places without an average household size, sets to the 2021 value (2.60)
...
HHincome_state.3 <- HHincome_state.2 %>% left_join(y = totalHH_state, by = "geoid") %>% #join with total # households
  ...
  HHEoE_perc = (HH_8feb24/hh_qualify)*100, ###NEED TO EDIT THIS ROW ANY TIME STATE DATA IS UPDATED###
  HH_eligiblePUMS = (hh_eligible/total_hh)*100,
  HHEoE_PUMSperc = (HH_8feb24/hh_eligible)*100,
  adj_factor = HHEoE_perc/HHEoE_PUMSperc) %>% 
  rename("totalHH"="total_hh", "HHqualify"="hh_qualify") %>%
  select(-name, -state) %>%
  select(geoid, State, totalHH, HHqualify, HHqualify_perc, HH_8feb24, HHEoE_perc,hh_eligible, HH_eligiblePUMS, HHEoE_PUMSperc, adj_factor) %>% 
...
#download household income data from acs
HHincome_zcta.1 <- get_acs(survey = "acs5", geography = "zcta", table = "B19001", year = 2022, geometry = FALSE, moe_level = 95, state = NULL,cache_table = TRUE) %>% clean_names()  #download version without geography 

#download household income data from acs
totalHH_zcta <- get_acs(survey = "acs5", geography = "zcta", variables = "B19001_001", year = 2022, geometry = FALSE, moe_level = 95, state = NULL,cache_table = TRUE) %>% clean_names() %>% select(geoid,estimate,moe) %>% rename("total_hh" = "estimate") %>%  #download version without geography 
mutate(total_hh = ifelse(total_hh == 0, moe, total_hh)) %>% select(-moe)
  
#download average household size from acs
HH.size_zcta <- get_acs(survey = "acs5", geography = "zcta", variables = "B25010_001", year = 2022, geometry = FALSE, moe_level = 95, state = NULL,cache_table = TRUE) %>% clean_names() %>% select(geoid,estimate) %>% rename("hh.size" = "estimate")
...
HHincome_zcta.2 <- HHincome_zcta.1  %>% select(-moe) %>% left_join(y = inc.max.vals, by = c("variable" = "var")) %>% 
 ...
 mutate(hh.size = if_else(is.na(hh.size) == TRUE, 2.57, hh.size), #https://www.census.gov/quickfacts/fact/table/US/HCN010217 average HH size 2017-2021
   l48.fpl = (p148 + (p248*(hh.size - 1))), #creates fpl based on l48, ak, and hi values, then selects the appropriate value for "fpl"
                                     ak.fpl = (p1ak + (p2ak*(hh.size - 1))),
                                     hi.fpl = (p1hi + (p2hi*(hh.size - 1))),
                                     fpl = if_else(region == "L48", l48.fpl,
...
ACP_Benton <- acp.service %>% right_join(y = bentonzips, by = "zipcode") %>% subset(is.na(city) == FALSE) %>% 
...
# #write to git folder
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_DASHBOARD_Benton_14may2024.shp", data = ACP_Benton_sp, overwrite = TRUE)
...
url <- "https://www.huduser.gov/hudapi/public/usps?type=5&query=All&year=2023"
...
# cdTOzcta <- read.csv("C:/Users/chris/ILSR Dropbox/Christine Parker/RawData/PUMS_5yr_2021/cdTOzcta.txt") %>% clean_names() %>%
...
# arc.write("C:/Users/chris/ILSR Dropbox/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/cd116", data = cd116)
...
cd_data <- read_excel("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Funding-Summary-by-Geography.xlsx", sheet = "CD_JAN") %>% clean_names()
...
#download household income data from acs
HHincome_cd.1 <- get_acs(survey = "acs5", geography = "congressional district", table = "B19001", year = 2022, geometry = FALSE, moe_level = 95, state = NULL,cache_table = TRUE) %>% clean_names()  #download version without geography 

#download household income data from acs
totalHH_cd <- get_acs(survey = "acs5", geography = "congressional district", variables = "B19001_001", year = 2022, geometry = FALSE, moe_level = 95, state = NULL,cache_table = TRUE) %>% clean_names() %>% select(geoid,estimate,moe) %>% rename("total_hh" = "estimate") %>%  #download version without geography 
mutate(total_hh = ifelse(total_hh == 0, moe, total_hh)) %>% select(-moe)
  
#download average household size from acs
HH.size_cd <- get_acs(survey = "acs5", geography = "congressional district", variables = "B25010_001", year = 2022, geometry = FALSE, moe_level = 95, state = NULL,cache_table = TRUE) %>% clean_names() %>% select(geoid,estimate) %>% rename("hh.size" = "estimate")
...
#join conversion file created in ArcGIS with eligibility data (116) and enrollment/claims data (118) (!!! update in ACS does not require to convert from 116)
ACP_116to118cd <-  HHincome_cd.3 %>%
  left_join(y = ACP_zip2cd, by = "geoid") %>%
  ungroup() %>% rename("cd118" = "geoid") %>%
  select(cd118, total_hh, hh_qualify, enrollments) %>% 
  group_by(cd118) %>%
  summarize(hh_qualify = sum(na.omit(hh_qualify)), total_hh = sum(na.omit(total_hh)), enrollments = sum(na.omit(enrollments))) %>%
  mutate(stab = substr(cd118, 1,2)) %>% 
  left_join(y = st_adj_factor[,c("state","STUSPS","GEOID", "adj_factor")], by = c("stab" = "GEOID")) %>% #join with state-level adjustment factor
  mutate(hh_qualify = na.omit(hh_qualify)*adj_factor) %>% #add in adjusted calculations of eligibility
  select(state, STUSPS, stab, cd118, total_hh, hh_qualify, enrollments) %>% 
  rename("state_code" = "stab", "district" = "cd118") %>% mutate(dist_code = substr(district, 3,4))
  
ACP_cd <- ACP_116to118cd %>% full_join(y = cd_data_all[,c("district","total_support")], by = "district") %>% #join with enrollment/claims data
...
ACP_cd_info <- legislators_188_simple %>% left_join(y = ACP_cd, by = c("state" = "STUSPS", "district" = "dist_code")) #add this dataset to ACP_DASHBOARD_V2 (ACP_cd_infoSP gets used for map)



#write to arcgis to join with cd spatial & write as shp to data folder (don't use tigris, unless it changes to 118)
#arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_CD14may24", data = ACP_cd_info, overwrite = TRUE)

# #write to git folder
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_CD14may24.shp", data = ACP_cd_info, overwrite = TRUE)
...
acp_ToPlot1 <- data.frame(acp_predict1$fit) %>% mutate(monthBYyear = 35:238) %>% rename(enrolled = fit)
...
#write.xlsx("C:/Users/chris/ILSR Dropbox/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/BroadbandFunds/ACPmodel.xlsx",
...
# #arc.write("C:/Users/chris/ILSR Dropbox/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/enroll35", data = enroll35b)
# 
...
enroll50b <- enroll50 %>% bind_rows(enroll50[rep(nrow(enroll50), 36),] %>%
              ...
              mutate(month.spent = enrolled*30, cum.spent = cumsum(month.spent),
                     perc.enrolled = (enrolled/US_eligibleHH)*100 ,perc.used = (cum.spent /ebb.acp)*100,
                     month = seq(as.Date("2022-01-01"), by = "month",length.out = 66)) %>%
  select(month, enrolled, lwr, upr,perc.enrolled,month.spent,cum.spent, perc.used) %>%
  mutate(month.spent25 = month.spent * 1.025,cum.spent25 = cumsum(month.spent25), perc.used25 = (cum.spent25 /ebb.acp)*100, model = "50 Percent Enrollment (26.1 Million Households)") 
enroll50b$month <- format(enroll50b$month, format = "%b-%y")
...
enroll55b <- enroll55 %>% bind_rows(enroll55[rep(nrow(enroll55), 36),] %>%
              ...
              mutate(month.spent = enrolled*30, cum.spent = cumsum(month.spent),
                     perc.enrolled = (enrolled/US_eligibleHH)*100 ,perc.used = (cum.spent /ebb.acp)*100,
                     month = seq(as.Date("2022-01-01"), by = "month",length.out = 70)) %>%
  select(month, enrolled, lwr, upr,perc.enrolled,month.spent,cum.spent, perc.used) %>%
...
enroll60b <- enroll60 %>% bind_rows(enroll60[rep(nrow(enroll60), 36),] %>%
              ...
              mutate(month.spent = enrolled*30, cum.spent = cumsum(month.spent),
                     perc.enrolled = (enrolled/US_eligibleHH)*100 ,perc.used = (cum.spent /ebb.acp)*100,
                     month = seq(as.Date("2022-01-01"), by = "month",length.out = 74)) %>%
  select(month, enrolled, lwr, upr,perc.enrolled,month.spent,cum.spent, perc.used) %>%
...
###numbers for White House
acp2024 <- dplyr::filter(modeldataCI, grepl("24",month)) %>% subset(model == "45 Percent Enrollment (23.3 Million Households)") %>% 
  mutate(needed = ifelse(str_detect(month, "Jan") == TRUE | str_detect(month, "Feb")== TRUE  | str_detect(month, "Mar")== TRUE | str_detect(month, "Apr")== TRUE, "covered", "fund"), without.device = ifelse(str_detect(month, "Jan") == FALSE | str_detect(month, "Feb")== FALSE  | str_detect(month, "Mar")== FALSE | str_detect(month, "Apr")== FALSE, month.spent25 - .01*month.spent25, month.spent25)) 

runs_out <- acp2024 %>% subset(needed == "fund") %>% mutate(with.device = cumsum(month.spent25), without.dev.total = cumsum(without.device))

needed <- acp2024 %>% group_by(needed) %>% summarise(with = sum(month.spent25), without = sum(without.device))
sum(claims22_24$device_support)/sum(claims22_24$total_support) #percent of device support
needed2 <- needed %>% mutate(without1 = with - .01*with)
```

```{r create dashboard workbook}
wb <- loadWorkbook("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_DASHBOARD_V2.xlsx")

#load in state data
removeWorksheet(wb, "EnrollByState")
addWorksheet(wb, "EnrollByState")
writeData(wb, "EnrollByState", HHincome_state.3)

#load in Benton data
removeWorksheet(wb, "BentonCitiesData")
addWorksheet(wb, "BentonCitiesData")
writeData(wb, "BentonCitiesData", ACP_Benton)

#load in model data
removeWorksheet(wb, "ModelData")
addWorksheet(wb, "ModelData")
writeData(wb, "ModelData", modeldata)

#load in zipcode data
removeWorksheet(wb, "EnrollByZipcode")
addWorksheet(wb, "EnrollByZipcode")
writeData(wb, "EnrollByZipcode", acp.service)

#congressional data
removeWorksheet(wb, "EnrollByCD")
addWorksheet(wb, "EnrollByCD")
writeData(wb, "EnrollByCD", ACP_cd_info)

saveWorkbook(wb,"C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_DASHBOARD_V2.xlsx",overwrite = TRUE)

#write zipcode shapefile
acp.zip.sp <- usZIP[,c("GEOID20", "geometry")] %>% left_join(y = acp.service, by = c("GEOID20" = "zcta"))
arc.write("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_ZipData_14may24.shp", data = acp.zip.sp, overwrite = TRUE) #may need to edit in future to ACP_ZipStateCD_geo

total_support <- sum(na.omit(claims22_24$total_support))

#write state boundary shapefile
#states <- arc.open("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/USA_StateBoundaries.shp") %>% arc.select() %>%
#arc.write("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/US_StateBoundaries.shp", data = states)
```  

```{r create workbook pineapple}

HHincome_state.3b <- HHincome_state.3 %>% mutate(estimated_spent_monthly = HH_8feb24*30*1.025) %>% 
  select(-HHqualify, -HHqualify_perc, -HHEoE_perc) %>%
  rename( "total_households_state" = "totalHH",
         "eligible_households_state" = "hh_PUMSeligible",
         "percent_eligible_households" = "HH_eligiblePUMS",
         "enrolled_households_district" = "HH_8feb24",
         "percent_enrolled_eligible" = "HHEoE_PUMSperc")
  
ACP_cd_info_pineapple <- ACP_cd_info %>%
  select(state, district, last_name, first_name, party, url, phone, total_hh:HHEoE) %>%
  rename("households_in_district" = "total_hh",
         "eligible_households_district" = "hh_qualify",
         "enrolled_households_district" = "enrollments",
         "spent_asof_june" = "total_support",
         "percent_eligible_households" = "percent_eligible",
         "percent_enrolled_eligible" = "HHEoE") %>% select(-state_name)


wb <- loadWorkbook("C:/Users/chris/ILSR Dropbox/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_8feb24.xlsx")
...
#congressional data
addWorksheet(wb, "EnrollByCD")
writeData(wb, "EnrollByCD", ACP_cd_info_pineapple)


saveWorkbook(wb,"C:/Users/chris/ILSR Dropbox/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_8feb24.xlsx",overwrite = TRUE)
...
```{r WA export}
#request from Alexis for advocacy in Washington state
acp.wa <- acp.service %>% subset(state == "WA") %>% 
  rename("HouseholdsInZip" = "totalHH", "EnrolledInZip" = "enrollments", "EligibleHouseholds" = "eligible_adj",
         "PercentEligible" = "eligible_adj_perc", "EnrolledOfEligible" = "HHEoE_adj", "TotalSpent" = "total_support") %>%
  select(-zipcode) %>% group_by(zcta, state, HouseholdsInZip, EligibleHouseholds, PercentEligible) %>%
  summarize(EnrolledInZip = sum(EnrolledInZip), EnrolledOfEligible = sum(na.omit(EnrolledOfEligible)), TotalSpent = sum(na.omit(TotalSpent)))

#write.csv("C:/Users/chris/ILSR Dropbox/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_WA_8feb24.csv", x = acp.wa)
```

```{r county data pull, echo=FALSE}

data(fips_codes) # get list of state & county fips codes

#import the list of zcta's for AK and HI
ak.counties <- fips_codes %>% subset(state_code == "02") %>% mutate(geoid = paste0(state_code,"",county_code)) #load AK counties
hi.counties <- fips_codes %>% subset(state_code == "15") %>% mutate(geoid = paste0(state_code,"",county_code)) #load HI counties 
...
#download household income data from acs
HHincome_county.1 <- get_acs(survey = "acs5", geography = "county", table = "B19001", year = 2021, geometry = FALSE, moe_level = 95, state = NULL,cache_table = FALSE) %>% clean_names()  #download version without geography 

#download household income data from acs
totalHH_county <- get_acs(survey = "acs5", geography = "county", variables = "B19001_001", year = 2021, geometry = FALSE, moe_level = 95, state = NULL,cache_table = FALSE) %>% clean_names() %>% select(geoid,estimate,moe) %>% rename("total_hh" = "estimate") %>%  #download version without geography 
mutate(total_hh = ifelse(total_hh == 0, moe, total_hh)) %>% select(-moe)
  
#download average household size from acs
HH.size_county <- get_acs(survey = "acs5", geography = "county", variables = "B25010_001", year = 2021, geometry = FALSE, moe_level = 95, state = NULL,cache_table = FALSE) %>% clean_names() %>% select(geoid,estimate) %>% rename("hh.size" = "estimate")

#combine acs income data with the max values of the income ranges, and average household size
HHincome_county.2 <- HHincome_county.1  %>% select(-moe) %>% left_join(y = inc.max.vals, by = c("variable" = "var")) %>% 
  mutate(acp_data = if_else(geoid %in% acp_county$geoid, "y","n"),
    region = if_else(geoid %in% ak.counties$geoid, "AK",
                if_else(geoid %in% hi.counties$geoid, "HI", "L48"))) %>%
  left_join(y = HH.size_county, by = "geoid") %>%
 mutate(hh.size = if_else(is.na(hh.size) == TRUE, 2.57, hh.size), #https://www.census.gov/quickfacts/fact/table/US/HCN010217 average HH size 2018-2022
   l48.fpl = (p148 + (p248*(hh.size - 1))), #creates fpl based on l48, ak, and hi values, then selects the appropriate value for "fpl"
                                     ak.fpl = (p1ak + (p2ak*(hh.size - 1))),
                                     hi.fpl = (p1hi + (p2hi*(hh.size - 1))),
                                     fpl = if_else(region == "L48", l48.fpl,
...
#join with total # households
HHincome_county.3 <- HHincome_county.2 %>% left_join(y = totalHH_county, by = "geoid") %>%
  mutate(HHqualify_perc = (hh_qualify/total_hh)*100) 



#write acp enrollment by zcta to gdb
acp.service.county <- acp_county %>% select(geoid,state_fips, county_fips, enrollments) %>%  
  subset(county_fips != "000") %>% #removes the row with redacted county data
  left_join(y = HHincome_county.3, by = "geoid") %>% 
  mutate(HHEoE_perc = (enrollments/hh_qualify*100)) %>% #calculate the percent of enrolled of eligible HH's
  rename("totalHH" = "total_hh") %>% 
  mutate(HHEoE_perc = ifelse(HHEoE_perc >= 100, 101,
                              ifelse(HHEoE_perc == Inf, 101,HHEoE_perc))) %>% #if % is greater than 100 or Inf, then set as 101 % 
  left_join(y = acp_county_spent, by = "geoid") %>%
  left_join(y = st_adj_factor[,c("STUSPS", "adj_factor")], by = c("state" = "STUSPS")) %>% 
  mutate(eligible_adj = hh_qualify*adj_factor, #add in adjusted calculations of eligibility
         eligible_adj_perc = (eligible_adj/totalHH)*100, 
         HHEoE_adj = (enrollments/eligible_adj)*100) %>%
  select(county_name,geoid,state,totalHH, enrollments,eligible_adj, eligible_adj_perc, HHEoE_adj, total_support) %>% 
  #subset(is.na(enrollments) == FALSE & is.na(HHEoE_adj) == FALSE) %>%
  mutate(HHEoE_adj = ifelse(HHEoE_adj >= 100, 101,
                              ifelse(HHEoE_adj == Inf, 101,HHEoE_adj)),
         eligible_adj_perc = ifelse(eligible_adj_perc >= 100, 101, eligible_adj_perc),
...
wb2 <- loadWorkbook("C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP4georgetown.xlsx")
#load in county data

removeWorksheet(wb2, "EnrollByCounty")
addWorksheet(wb2, "EnrollByCounty")
writeData(wb2, "EnrollByCounty", acp.service.county)

saveWorkbook(wb2,"C:/Users/chris/ILSR Dropbox/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP4georgetown.xlsx",overwrite = TRUE)
