...
ACP2oct <- "C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACPWeeklyEnroll.xlsx"
...
claims2023 <- ACP_Claims_by_Zip_January_June_2023 <- read_excel("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Claims-by-Zip-January-August-2023.xlsx", 
...
ACPstates <-read_excel(path = ACP2oct, sheet = 4, col_names = TRUE) %>%
  select(State,HH_2oct23) %>% 
...
#-------------------------------------------------------
#NATIONAL DATA
acp_national <-read_excel(path = ACP2oct, sheet = 2, col_names = TRUE) %>% clean_names() %>%
...
future <- data.frame(monthBYyear = c(31:238))
...
HHincome_state.3 <- HHincome_state.2 %>% left_join(y = totalHH_state, by = "geoid") %>% #join with total # households
  ...
  HHEoE_perc = (HH_2oct23/hh_qualify)*100, ###NEED TO EDIT THIS ROW ANY TIME STATE DATA IS UPDATED###
  HH_eligiblePUMS = (hh_eligible/total_hh)*100,
  HHEoE_PUMSperc = (HH_2oct23/hh_eligible)*100,
  adj_factor = HHEoE_perc/HHEoE_PUMSperc) %>% 
  rename("totalHH"="total_hh", "HHqualify"="hh_qualify") %>%
  select(-name, -state) %>%
  select(geoid, State, totalHH, HHqualify, HHqualify_perc, HH_2oct23, HHEoE_perc,hh_eligible, HH_eligiblePUMS, HHEoE_PUMSperc, adj_factor) %>% 
...
# #write to git folder
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_DASHBOARD_Benton_2oct2023.shp", data = ACP_Benton_sp, overwrite = TRUE)
...
cd_data <- read_excel("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP-Funding-Summary-by-Geography.xlsx", sheet = "CD_AUG") %>% clean_names()
...
#arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/ACP/ACP.gdb/ACP_CD2oct23", data = ACP_cd_info, overwrite = TRUE)

# #write to git folder
# arc.write("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_CD2oct23.shp", data = ACP_cd_info, overwrite = TRUE)
...
acp_ToPlot1 <- data.frame(acp_predict1$fit) %>% mutate(monthBYyear = 31:238) %>% rename(enrolled = fit)
...
enroll.currentb <- enroll.current %>% bind_rows(enroll.current[rep(nrow(enroll.current), 36),] %>%
              ...
              mutate(month.spent = enrolled*30, cum.spent = cumsum(month.spent), #estimates monthly spent (based only on enrollment & cumulative spent)
                     ...
                     month = seq(as.Date("2022-01-01"), by = "month",length.out = 58)) %>%
...
arc.write("C:/Users/chris/Dropbox (ILSR)/CBN Team Folder/Resources/Resources/Affordable Connectivity Program/Dashboard Data/ACP_ZipData_2oct.shp", data = acp.zip.sp, overwrite = TRUE) #may need to edit in future to ACP_ZipStateCD_geo
...
HHincome_state.3b <- HHincome_state.3 %>% mutate(estimated_spent_monthly = HH_2oct23*30*1.025) %>% 
  ...
  rename( "total_households_state" = "totalHH",
         ...
         "enrolled_households_district" = "HH_2oct23",
...
wb <- loadWorkbook("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_2oct23.xlsx")
...
saveWorkbook(wb,"C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_QuickFactsData_2oct23.xlsx",overwrite = TRUE)
...
#write.csv("C:/Users/chris/Dropbox (ILSR)/Christine Parker/Projects/CBN InfoMap Requests/BroadbandingFunds/ACP/Affordable-Connectivity-Program-Analysis/ACP_WA_2oct23.csv", x = acp.wa)
```