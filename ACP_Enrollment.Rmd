...
ACP20June <- "C:/Users/Michelle/Dropbox (ILSR)/Christine Parker/RawData/ACP/ACPWeeklyEnroll.xlsx"
ACP <-read_excel(path = ACP20June, sheet = 2, col_names = TRUE) %>% clean_names() %>%
...
ACPstates <-read_excel(path = "C:/Users/Michelle/Dropbox (ILSR)/Christine Parker/RawData/ACP/ACPWeeklyEnroll.xlsx", sheet = 3, col_names = TRUE)  %>%
  right_join(y = HHincome.3, by = c("State" = "name" )) %>% clean_names() %>% mutate(HHEoE_perc
 = (hh_june20/h_hincome)*100) %>% ###NEED TO EDIT THIS ROW ANY TIME STATE DATA IS UPDATED###
...
write.xlsx("G:/My Drive/ACPdashboard/ACP_enroll.xlsx", sheetName = "EnrollByState", x = ACPstates, append = TRUE)
...
qualHH75 <- qualHH*.75
acp_ToPlot1 <- data.frame(acp_predict1$fit) %>% mutate(month = 7:232) %>% rename(enrolled = fit)#create index which represents months - currently predicting May 2022 and beyond
...
enroll35b <- enroll35 %>% bind_rows(enroll35[rep(nrow(enroll35), 36),] %>%
  ...
  mutate(cum.spent25 = cum.spent*1.025) %>%
